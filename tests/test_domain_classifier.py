...
class TestDeterministicClassification:
    ...
    def test_social_science_daycare(self):
        result = classify_topic_deterministic("Effects of daycare vs stay-at-home parenting on child development")
        assert result is not None
        assert result.domain == ResearchDomain.SOCIAL_SCIENCE

    def test_social_science_remote_work(self):
        result = classify_topic_deterministic("Does remote work increase employee productivity and job satisfaction?")
        assert result is not None
        assert result.domain == ResearchDomain.SOCIAL_SCIENCE
    ...
    def test_strong_clinical_multiple_keywords(self):
        result = classify_topic_deterministic("Randomized clinical trial of aspirin for heart disease patients")
        assert result is not None
        assert result.domain == ResearchDomain.CLINICAL
...
class TestAsyncClassification:
    def test_clinical_topic_classified(self):
        result = asyncio.run(classify_topic("Metformin treatment for diabetes patients"))
        assert result.domain == ResearchDomain.CLINICAL

    def test_social_science_topic_classified(self):
        result = asyncio.run(classify_topic("Effects of homework on student academic achievement"))
        assert result.domain == ResearchDomain.SOCIAL_SCIENCE

    def test_ambiguous_defaults_to_clinical(self):
        """Without LLM client, ambiguous topics default to CLINICAL."""
        result = asyncio.run(classify_topic("How does music affect the brain?"))
        assert result.domain == ResearchDomain.CLINICAL
    ...
    def test_deterministic_bypasses_llm(self):
        ...
        result = asyncio.run(
            classify_topic(
                "Randomized clinical trial of aspirin for heart disease patients", smart_client=None, smart_model=""
            )
        )
        assert result.domain == ResearchDomain.CLINICAL
        assert result.confidence >= 0.6
